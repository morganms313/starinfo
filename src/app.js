(function () {
  ...
  // ---- constants -----------------------------------------------------------
  // Tree is rendered from the text output (same human-readable strings as the desktop
  // app's tree), so Text and Tree share one parse and one cache entry.
  var FORMAT = { text: 'text', tree: 'text', json: 'JSON', xml: 'XML', html: 'HTML' };
  ...
  function noteLibVersion(view, res) {
    if (state.libVersion || view !== 'json') return;
    try {
  ...
  function showResult(res) {
    if (state.view === 'tree') { els.report.appendChild(buildTree(parseMediaInfoText(res))); return; }
    if (state.view === 'html') {
  ...
  // Tree: one collapsible section per track (General, Video, Audio #1, ...), rows of
  // field -> value, taken from parseMediaInfoText() so values match the Text view.
  function buildTree(sections) {
    ...
    sections.forEach(function (s) {
      ...
      var sum = document.createElement('summary'); sum.textContent = s.title || 'Track';
      det.appendChild(sum);
      var table = document.createElement('table');
      s.rows.forEach(function (row) {
        var tr = document.createElement('tr');
        var td1 = document.createElement('td'); td1.className = 'k'; td1.textContent = row[0];
    ...
    if (sections.length === 0) { var p = document.createElement('p'); p.className = 'loading'; p.textContent = 'No tracks found.'; root.appendChild(p); }
    return root;
  }
